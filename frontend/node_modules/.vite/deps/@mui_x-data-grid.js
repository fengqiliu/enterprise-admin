...
var useGridPaginationModel = (apiRef, props) => {
  ...
  const handleRowCountChange = React104.useCallback((newRowCount) => {
    if (newRowCount == null) {
      return;
    }
    const paginationModel = gridPaginationModelSelector(apiRef);
  ...
  React104.useEffect(handleUpdateAutoPageSize, [handleUpdateAutoPageSize]);
};
...
var useGridRowCount = (apiRef, props) => {
  ...
  const setRowCount = React109.useCallback((newRowCount) => {
...
var useGridPreferencesPanel = (apiRef, props) => {
  ...
  const hideTimeout = React110.useRef();
  const immediateTimeout = React110.useRef();
  const hidePreferences = React110.useCallback(() => {
  ...
  const doNotHidePanel = React110.useCallback(() => {
    immediateTimeout.current = setTimeout(() => clearTimeout(hideTimeout.current), 0);
  }, []);
  const hidePreferencesDelayed = React110.useCallback(() => {
    hideTimeout.current = setTimeout(hidePreferences, 100);
  }, [hidePreferences]);
  const showPreferences = React110.useCallback((newValue, panelId, labelId) => {
  ...
  const stateExportPreProcessing = React110.useCallback((prevState, context) => {
    var _props$initialState2;
  ...
  const stateRestorePreProcessing = React110.useCallback((params, context) => {
...
var useGridCellEditing = (apiRef, props) => {
  const [cellModesModel, setCellModesModel] = React111.useState({});
  const cellModesModelRef = React111.useRef(cellModesModel);
  const prevCellModesModel = React111.useRef({});
  const {
    processRowUpdate,
  ...
  const throwIfNotInMode = React111.useCallback((id, field, mode) => {
  ...
  const handleCellEditStart = React111.useCallback((params) => {
    const {
      id,
      field,
      reason
  ...
  const handleCellEditStop = React111.useCallback((params) => {
    const {
      id,
      field,
      reason
  ...
  const getCellMode = React111.useCallback((id, field) => {
  ...
  const updateFieldInCellModesModel = React111.useCallback((id, field, newProps) => {
  ...
  const startCellEditMode = React111.useCallback((params) => {
    const {
      id,
      field
  ...
  const stopCellEditMode = React111.useCallback((params) => {
    const {
      id,
      field
  ...
  const setCellEditingEditCellValue = React111.useCallback(async (params) => {
  ...
  const getRowWithUpdatedValuesFromCellEditing = React111.useCallback((id, field) => {
    const column = apiRef.current.getColumn(field);
    const editingState = gridEditRowsStateSelector(apiRef.current.state);
    const row = apiRef.current.getRow(id);
...
var useGridRowEditing = (apiRef, props) => {
  const [rowModesModel, setRowModesModel] = React112.useState({});
  const rowModesModelRef = React112.useRef(rowModesModel);
  const prevRowModesModel = React112.useRef({});
  const focusTimeout = React112.useRef(null);
  const nextFocusedCell = React112.useRef(null);
  const {
    processRowUpdate,
    onProcessRowUpdateError,
  ...
  const throwIfNotEditable = React112.useCallback((id, field) => {
    const params = apiRef.current.getCellParams(id, field);
    if (!apiRef.current.isCellEditable(params)) {
      throw new Error(`MUI: The cell with id=${id} and field=${field} is not editable.`);
    }
  }, [apiRef]);
  const throwIfNotInMode = React112.useCallback((id, mode) => {
    if (apiRef.current.getRowMode(id) !== mode) {
      throw new Error(`MUI: The row with id=${id} is not in ${mode} mode.`);
    }
  }, [apiRef]);
  const handleCellDoubleClick = React112.useCallback((params, event) => {
    if (!params.isEditable) {
      return;
    }
  ...
  const handleCellFocusIn = React112.useCallback((params) => {
    nextFocusedCell.current = params;
  }, []);
  const handleCellFocusOut = React112.useCallback((params, event) => {
    if (!params.isEditable) {
      return;
    }
  ...
  const handleCellKeyDown = React112.useCallback((params, event) => {
  ...
  const handleRowEditStart = React112.useCallback((params) => {
    const {
      id,
      field,
      reason
  ...
  const handleRowEditStop = React112.useCallback((params) => {
    const {
      id,
      reason,
      field
  ...
  const getRowMode = React112.useCallback((id) => {
    if (props.editMode === GridEditModes.Cell) {
  ...
  const updateRowInRowModesModel = React112.useCallback((id, newProps) => {
  ...
  const updateOrDeleteRowState = React112.useCallback((id, newProps) => {
    apiRef.current.setState((state) => {
      const newEditingState = _extends({}, state.editRows);
      if (newProps !== null) {
  ...
  const updateOrDeleteFieldState = React112.useCallback((id, field, newProps) => {
    apiRef.current.setState((state) => {
      const newEditingState = _extends({}, state.editRows);
      if (newProps !== null) {
        newEditingState[id] = _extends({}, newEditingState[id], {
  ...
  const startRowEditMode = React112.useCallback((params) => {
    const {
      id
  ...
  const stopRowEditMode = React112.useCallback((params) => {
    const {
      id
  ...
  const setRowEditingEditCellValue = React112.useCallback((params) => {
    const {
      id,
      field,
      value,
  ...
  const getRowWithUpdatedValuesFromRowEditing = React112.useCallback((id) => {
    const editingState = gridEditRowsStateSelector(apiRef.current.state);
    const row = apiRef.current.getRow(id);
    if (!editingState[id]) {
      return apiRef.current.getRow(id);
...
var useGridEditing = (apiRef, props) => {
  ...
  const debounceMap = React113.useRef({});
  const {
    isCellEditable: isCellEditableProp
  } = props;
  const isCellEditable = React113.useCallback((params) => {
    if (isAutoGeneratedRow(params.rowNode)) {
      return false;
    }
  ...
  const runPendingEditCellValueMutation = React113.useCallback((id, field) => {
    if (!debounceMap.current[id]) {
      return;
    }
    if (!field) {
  ...
  const setEditCellValue = React113.useCallback((params) => {
    const {
      id,
      field,
      debounceMs
  ...
  const getRowWithUpdatedValues = React113.useCallback((id, field) => {
    return props.editMode === GridEditModes.Cell ? apiRef.current.getRowWithUpdatedValuesFromCellEditing(id, field) : apiRef.current.getRowWithUpdatedValuesFromRowEditing(id);
  }, [apiRef, props.editMode]);
  const getEditCellMeta = React113.useCallback((id, field) => {
...
var useGridRows = (apiRef, props) => {
  ...
  const lastUpdateMs = React114.useRef(Date.now());
  const timeout = useTimeout();
  const getRow = React114.useCallback((id) => {
  ...
  const getRowId = React114.useCallback((row) => {
  ...
  const lookup = React114.useMemo(() => currentPage.rows.reduce((acc, {
    id
  }, index) => {
    acc[id] = index;
    return acc;
  }, {}), [currentPage.rows]);
  const throttledRowsChange = React114.useCallback(({
    cache,
    throttle
  }) => {
    const run = () => {
  ...
  const setRows = React114.useCallback((rows) => {
  ...
  const updateRows = React114.useCallback((updates) => {
  ...
  const getRowModels = React114.useCallback(() => {
  ...
  const getRowsCount = React114.useCallback(() => gridRowCountSelector(apiRef), [apiRef]);
  const getAllRowIds = React114.useCallback(() => gridDataRowIdsSelector(apiRef), [apiRef]);
  const getRowIndexRelativeToVisibleRows = React114.useCallback((id) => lookup[id], [lookup]);
  const setRowChildrenExpansion = React114.useCallback((id, isExpanded) => {
  ...
  const getRowNode = React114.useCallback((id) => {
    var _ref;
    return (_ref = gridRowTreeSelector(apiRef)[id]) != null ? _ref : null;
  }, [apiRef]);
  const getRowGroupChildren = React114.useCallback(({
  ...
  const setRowIndex = React114.useCallback((rowId, targetIndex) => {
  ...
  const replaceRows = React114.useCallback((firstRowToRender, newRows) => {
  ...
  const groupRows = React114.useCallback(() => {
  ...
  const handleStrategyActivityChange = React114.useCallback(() => {
  ...
  const applyHydrateRowsProcessor = React114.useCallback(() => {
    apiRef.current.setState((state) => {
...
var useGridRowSelection = (apiRef, props) => {
  ...
  const propRowSelectionModel = React115.useMemo(() => {
  ...
  const setRowSelectionModel = React115.useCallback((model) => {
  ...
  const isRowSelected = React115.useCallback((id) => gridRowSelectionStateSelector(apiRef.current.state).includes(id), [apiRef]);
  ...
  const handleRowClick = React115.useCallback((params, event) => {
    var _closest;
...
var useGridRowSelectionPreProcessors = (apiRef, props) => {
  ...
  const updateSelectionColumn = React116.useCallback((columnsState) => {
...
var useGridSorting = (apiRef, props) => {
  ...
  const upsertSortModel = React117.useCallback((field, sortItem) => {
  ...
  const createSortItem = React117.useCallback((col, directionOverride) => {
  ...
  const addColumnMenuItem = React117.useCallback((columnMenuItems, colDef) => {
  ...
  const applySorting = React117.useCallback(() => {
    apiRef.current.setState((state) => {
  ...
  const setSortModel = React117.useCallback((model) => {
  ...
  const sortColumn = React117.useCallback((column, direction, allowMultipleSorting) => {
    if (!column.sortable) {
      return;
    }
  ...
  const getSortModel = React117.useCallback(() => gridSortModelSelector(apiRef), [apiRef]);
  const getSortedRows = React117.useCallback(() => {
    const sortedRows = gridSortedRowEntriesSelector(apiRef);
    return sortedRows.map((row) => row.model);
  }, [apiRef]);
  const getSortedRowIds = React117.useCallback(() => gridSortedRowIdsSelector(apiRef), [apiRef]);
  const getRowIdFromRowIndex = React117.useCallback((index) => apiRef.current.getSortedRowIds()[index], [apiRef]);
  const sortApi = {
    getSortModel,
    getSortedRows,
    getSortedRowIds,
  ...
  const flatSortingMethod = React117.useCallback((params) => {
    const rowTree = gridRowTreeSelector(apiRef);
  ...
  const handleColumnHeaderClick = React117.useCallback(({
    colDef
  }, event) => {
    const allowMultipleSorting = event.shiftKey || event.metaKey || event.ctrlKey;
    sortColumn(colDef, void 0, allowMultipleSorting);
  }, [sortColumn]);
  const handleColumnHeaderKeyDown = React117.useCallback(({
    colDef
  }, event) => {
  ...
  const handleColumnsChange = React117.useCallback(() => {
    const sortModel = gridSortModelSelector(apiRef);
  ...
  const handleStrategyProcessorChange = React117.useCallback((methodName) => {
...
var useGridScroll = (apiRef, props) => {
  ...
  const scrollToIndexes = React118.useCallback((params) => {
  ...
  const scroll = React118.useCallback((params) => {
  ...
  const getScrollPosition = React118.useCallback(() => {
...
function useGridDimensions(apiRef, props) {
  ...
  const errorShown = React119.useRef(false);
  const rootDimensionsRef = React119.useRef(null);
  const fullDimensionsRef = React119.useRef(null);
  const rowsMeta = useGridSelector(apiRef, gridRowsMetaSelector);
  const densityFactor = useGridSelector(apiRef, gridDensityFactorSelector);
  const rowHeight = Math.floor(props.rowHeight * densityFactor);
  const totalHeaderHeight = getTotalHeaderHeight(apiRef, props.columnHeaderHeight);
  const updateGridDimensionsRef = React119.useCallback(() => {
    var _apiRef$current$rootE;
  ...
  const [savedSize, setSavedSize] = React119.useState();
  const debouncedSetSavedSize = React119.useMemo(() => debounce(setSavedSize, 60), []);
  const previousSize = React119.useRef();
  useEnhancedEffect_default(() => {
    if (savedSize) {
  ...
  const resize = React119.useCallback(() => {
    apiRef.current.computeSizeAndPublishResizeEvent();
  }, [apiRef]);
  const getRootDimensions = React119.useCallback(() => fullDimensionsRef.current, []);
  const getViewportPageSize = React119.useCallback(() => {
    const dimensions = apiRef.current.getRootDimensions();
    if (!dimensions) {
      return 0;
    }
  ...
  const computeSizeAndPublishResizeEvent = React119.useCallback(() => {
  ...
  const isFirstSizing = React119.useRef(true);
  const handleResize = React119.useCallback((size) => {
...
var useGridVirtualScroller = (props) => {
  ...
  const currentPage = useGridVisibleRows(apiRef, rootProps);
  const renderZoneRef = React120.useRef(null);
  const rootRef = React120.useRef(null);
  const handleRef = useForkRef(ref, rootRef);
  const [renderContext, setRenderContextState] = React120.useState(null);
  const prevRenderContext = React120.useRef(renderContext);
  const scrollPosition = React120.useRef({
    top: 0,
    left: 0
  });
  const [containerDimensions, setContainerDimensions] = React120.useState({
    width: null,
    height: null
  });
  const prevTotalWidth = React120.useRef(columnsTotalWidth);
  const [hoveredRowId, setHoveredRowId] = React120.useState(null);
  const rowStyleCache = React120.useRef(/* @__PURE__ */ Object.create(null));
  const prevGetRowProps = React120.useRef();
  const prevRootRowStyle = React120.useRef();
  const getRenderedColumnsRef = React120.useRef(defaultMemoize((columns, firstColumnToRender, lastColumnToRender, minFirstColumn, maxLastColumn, indexOfColumnWithFocusedCell2) => {
  ...
  const indexOfColumnWithFocusedCell = React120.useMemo(() => {
    if (cellFocus !== null) {
      return visibleColumns.findIndex((column) => column.field === cellFocus.field);
    }
    return -1;
  }, [cellFocus, visibleColumns]);
  const computeRenderContext = React120.useCallback(() => {
    if (!enabled) {
      return {
        firstRowIndex: 0,
  ...
  const handleResize = React120.useCallback(() => {
    if (rootRef.current) {
      setContainerDimensions({
        width: rootRef.current.clientWidth,
        height: rootRef.current.clientHeight
  ...
  const updateRenderZonePosition = React120.useCallback((nextRenderContext) => {
    const [firstRowToRender, lastRowToRender] = getRenderableIndexes({
  ...
  const getRenderContext = React120.useCallback(() => prevRenderContext.current, []);
  ...
  const indexOfRowWithFocusedCell = React120.useMemo(() => {
    if (cellFocus !== null) {
  ...
  const contentSize = React120.useMemo(() => {
  ...
  const rootStyle = React120.useMemo(() => {
    const style = {};
...
function GridColumnGroupHeader(props) {
  ...
  const rootProps = useGridRootProps();
  const headerCellRef = React121.useRef(null);
  const apiRef = useGridApiContext();
  ...
  const renderParams = React121.useMemo(() => ({
    groupId,
    headerName,
    description,
    depth,
  ...
  const publish = React121.useCallback(
    (eventName) => (event) => {
      if (isEventTargetInPortal(event)) {
        return;
      }
  ...
  const mouseEventsHandlers = React121.useMemo(() => ({
...
var useGridColumnHeaders = (props) => {
  ...
  const theme = useTheme();
  const [dragCol, setDragCol] = React122.useState("");
  const [resizeCol, setResizeCol] = React122.useState("");
  const apiRef = useGridPrivateApiContext();
  const hasVirtualization = useGridSelector(apiRef, gridVirtualizationColumnEnabledSelector);
  const rootProps = useGridRootProps();
  const innerRef = React122.useRef(null);
  const handleInnerRef = useForkRef(innerRefProp, innerRef);
  const [renderContext, setRenderContextRaw] = React122.useState(null);
  const prevRenderContext = React122.useRef(renderContext);
  const prevScrollLeft = React122.useRef(0);
  const currentPage = useGridVisibleRows(apiRef, rootProps);
  const totalHeaderHeight = getTotalHeaderHeight(apiRef, rootProps.columnHeaderHeight);
  const headerHeight = Math.floor(rootProps.columnHeaderHeight * densityFactor);
  const setRenderContext = React122.useCallback((nextRenderContext) => {
  ...
  const getFirstColumnIndexToRenderRef = React122.useRef(defaultMemoize(getFirstColumnIndexToRender, {
    equalityCheck: (a, b) => ["firstColumnIndex", "minColumnIndex", "columnBuffer"].every((key) => a[key] === b[key])
  }));
  const updateInnerPosition = React122.useCallback((nextRenderContext) => {
    const [firstRowToRender, lastRowToRender] = getRenderableIndexes({
      firstIndex: nextRenderContext.firstRowIndex,
      lastIndex: nextRenderContext.lastRowIndex,
      minFirstIndex: 0,
  ...
  const handleScroll = React122.useCallback(({
    left,
  ...
  const handleColumnResizeStart = React122.useCallback((params) => setResizeCol(params.field), []);
  const handleColumnResizeStop = React122.useCallback(() => setResizeCol(""), []);
  const handleColumnReorderStart = React122.useCallback((params) => setDragCol(params.field), []);
  const handleColumnReorderStop = React122.useCallback(() => setDragCol(""), []);
...
var GridBaseColumnHeaders = React123.forwardRef(function GridColumnHeaders(props, ref) {
  const {
    className
...
var GridColumnHeadersInner = React124.forwardRef(function GridColumnHeadersInner2(props, ref) {
  var _apiRef$current$getRo, _apiRef$current$getRo2;
  const {
    isDragging,
    className
...
var GridColumnHeaders2 = React125.forwardRef(function GridColumnsHeaders(props, ref) {
  const {
    innerRef,
    visibleColumns,
    sortColumnLookup,
...
true ? GridColumnHeaders2.propTypes = {
  ...
  // | To update them edit the TypeScript types and run "yarn proptypes"  |
  // ----------------------------------------------------------------------
  columnGroupHeaderFocus: import_prop_types57.default.shape({
    depth: import_prop_types57.default.number.isRequired,
    field: import_prop_types57.default.string.isRequired
  }),
  columnGroupHeaderTabIndexState: import_prop_types57.default.shape({
    depth: import_prop_types57.default.number.isRequired,
    field: import_prop_types57.default.string.isRequired
  }),
  columnGroupsHeaderStructure: import_prop_types57.default.arrayOf(import_prop_types57.default.arrayOf(import_prop_types57.default.shape({
    columnFields: import_prop_types57.default.arrayOf(import_prop_types57.default.string).isRequired,
    groupId: import_prop_types57.default.string
  }))).isRequired,
  columnHeaderFocus: import_prop_types57.default.shape({
    field: import_prop_types57.default.string.isRequired
  }),
  columnHeaderTabIndexState: import_prop_types57.default.shape({
    field: import_prop_types57.default.string.isRequired
  }),
  columnMenuState: import_prop_types57.default.shape({
    field: import_prop_types57.default.string,
    open: import_prop_types57.default.bool.isRequired
  }).isRequired,
  columnPositions: import_prop_types57.default.arrayOf(import_prop_types57.default.number).isRequired,
  columnVisibility: import_prop_types57.default.object.isRequired,
  densityFactor: import_prop_types57.default.number.isRequired,
  filterColumnLookup: import_prop_types57.default.object.isRequired,
  hasOtherElementInTabSequence: import_prop_types57.default.bool.isRequired,
  headerGroupingMaxDepth: import_prop_types57.default.number.isRequired,
  innerRef: refType_default,
  minColumnIndex: import_prop_types57.default.number,
  sortColumnLookup: import_prop_types57.default.object.isRequired,
  visibleColumns: import_prop_types57.default.arrayOf(import_prop_types57.default.object).isRequired
} : void 0;
...
var GridNoResultsOverlay = React126.forwardRef(function GridNoResultsOverlay2(props, ref) {
  const apiRef = useGridApiContext();
...
var GridColumnUnsortedIcon = React127.memo(function GridColumnHeaderSortIcon2(props) {
  const {
    sortingOrder
...
var useDataGridProps = (inProps) => {
  ...
  const slots = React130.useMemo(() => computeSlots({
    defaultSlots,
...
var useGridRowsMeta = (apiRef, props) => {
  ...
  const rowsHeightLookup = React131.useRef(/* @__PURE__ */ Object.create(null));
  const lastMeasuredRowIndex = React131.useRef(-1);
  const hasRowWithAutoHeight = React131.useRef(false);
  const densityFactor = useGridSelector(apiRef, gridDensityFactorSelector);
  const filterModel = useGridSelector(apiRef, gridFilterModelSelector);
  ...
  const hydrateRowsMeta = React131.useCallback(() => {
    var _pinnedRows$top, _pinnedRows$bottom;
  ...
  const getRowHeight = React131.useCallback((rowId) => {
  ...
  const setRowHeight = React131.useCallback((id, height) => {
    rowsHeightLookup.current[id].sizes.baseCenter = height;
    rowsHeightLookup.current[id].isResized = true;
    rowsHeightLookup.current[id].needsFirstMeasurement = false;
    hydrateRowsMeta();
  }, [hydrateRowsMeta]);
  const debouncedHydrateRowsMeta = React131.useMemo(() => debounce(hydrateRowsMeta, props.rowPositionsDebounceMs), [hydrateRowsMeta, props.rowPositionsDebounceMs]);
  const storeMeasuredRowHeight = React131.useCallback((id, height, position) => {
  ...
  const rowHasAutoHeight = React131.useCallback((id) => {
    var _rowsHeightLookup$cur2;
    return ((_rowsHeightLookup$cur2 = rowsHeightLookup.current[id]) == null ? void 0 : _rowsHeightLookup$cur2.autoHeight) || false;
  }, []);
  const getLastMeasuredRowIndex = React131.useCallback(() => {
    return lastMeasuredRowIndex.current;
  }, []);
  const setLastMeasuredRowIndex = React131.useCallback((index) => {
    if (hasRowWithAutoHeight.current && index > lastMeasuredRowIndex.current) {
      lastMeasuredRowIndex.current = index;
    }
  }, []);
  const resetRowHeights = React131.useCallback(() => {
...
var useGridStatePersistence = (apiRef) => {
  const exportState = React132.useCallback((params = {}) => {
    const stateToExport = apiRef.current.unstable_applyPipeProcessors("exportState", {}, params);
    return stateToExport;
  }, [apiRef]);
  const restoreState = React132.useCallback((stateToRestore) => {
...
var useGridColumnSpanning = (apiRef) => {
  const lookup = React133.useRef({});
  const setCellColSpanInfo = React133.useCallback((rowId, columnIndex, cellColSpanInfo) => {
  ...
  const getCellColSpanInfo = React133.useCallback((rowId, columnIndex) => {
    var _lookup$current$rowId;
    return (_lookup$current$rowId = lookup.current[rowId]) == null ? void 0 : _lookup$current$rowId[columnIndex];
  }, []);
  const calculateCellColSpan = React133.useCallback((params) => {
    const {
      columnIndex,
      rowId,
      minFirstColumnIndex,
  ...
  const calculateColSpan = React133.useCallback(({
    rowId,
    minFirstColumn,
    maxLastColumn,
    columns
  ...
  const handleColumnReorderChange = React133.useCallback(() => {
    lookup.current = {};
  }, []);
...
var useGridColumnGrouping = (apiRef, props) => {
  ...
  const getColumnGroupPath = React134.useCallback((field) => {
    var _unwrappedGroupingMod3;
    const unwrappedGroupingModel = gridColumnGroupsUnwrappedModelSelector(apiRef);
    return (_unwrappedGroupingMod3 = unwrappedGroupingModel[field]) != null ? _unwrappedGroupingMod3 : [];
  }, [apiRef]);
  const getAllGroupDetails = React134.useCallback(() => {
  ...
  const handleColumnIndexChange = React134.useCallback(() => {
  ...
  const updateColumnGroupingState = React134.useCallback((columnGroupingModel) => {
...
var GridVirtualScroller = React135.forwardRef(function GridVirtualScroller2(props, ref) {
  const rootProps = useGridRootProps();
...
var GridVirtualScrollerContent = React136.forwardRef(function GridVirtualScrollerContent2(props, ref) {
  var _props$style;
  const rootProps = useGridRootProps();
...
var GridVirtualScrollerRenderZone = React137.forwardRef(function GridVirtualScrollerRenderZone2(props, ref) {
  const {
    className
...
var DataGridVirtualScroller = React138.forwardRef(function DataGridVirtualScroller2(props, ref) {
  const {
    className
...
var DataGridRaw = React139.forwardRef(function DataGrid(inProps, ref) {
...
var DataGrid2 = React139.memo(DataGridRaw);
...
DataGridRaw.propTypes = {
   ...
   * @default false
   */
  autoHeight: import_prop_types58.default.bool,
  /**
   * If `true`, the pageSize is calculated according to the container size and the max number of rows to avoid rendering a vertical scroll bar.
   * @default false
   */
  autoPageSize: import_prop_types58.default.bool,
  /**
   * Controls the modes of the cells.
   */
  cellModesModel: import_prop_types58.default.object,
  /**
   * If `true`, the Data Grid will display an extra column with checkboxes for selecting rows.
   * @default false
   */
  checkboxSelection: import_prop_types58.default.bool,
  /**
   * Override or extend the styles applied to the component.
   */
   ...
   * @default '\t'
   */
  clipboardCopyCellDelimiter: import_prop_types58.default.string,
  /**
   * Number of extra columns to be rendered before/after the visible slice.
   * @default 3
   */
  columnBuffer: import_prop_types58.default.number,
  columnGroupingModel: import_prop_types58.default.arrayOf(import_prop_types58.default.object),
  /**
   * Sets the height in pixel of the column headers in the Data Grid.
   * @default 56
   */
  columnHeaderHeight: import_prop_types58.default.number,
  /**
   * Set of columns of type [[GridColDef[]]].
   */
  columns: chainPropTypes(import_prop_types58.default.array.isRequired, (props) => {
   ...
   * @default 3
   */
  columnThreshold: import_prop_types58.default.number,
  /**
   * Set the column visibility model of the Data Grid.
   * If defined, the Data Grid will ignore the `hide` property in [[GridColDef]].
   */
  columnVisibilityModel: import_prop_types58.default.object,
  /**
   ...
   * @default "standard"
   */
  density: import_prop_types58.default.oneOf(["comfortable", "compact", "standard"]),
  /**
   * If `true`, column filters are disabled.
   * @default false
   */
  disableColumnFilter: import_prop_types58.default.bool,
  /**
   * If `true`, the column menu is disabled.
   * @default false
   */
  disableColumnMenu: import_prop_types58.default.bool,
  /**
   * If `true`, hiding/showing columns is disabled.
   * @default false
   */
  disableColumnSelector: import_prop_types58.default.bool,
  /**
   * If `true`, the density selector is disabled.
   * @default false
   */
  disableDensitySelector: import_prop_types58.default.bool,
  /**
   * If `true`, `eval()` is not used for performance optimization.
   * @default false
   */
  disableEval: import_prop_types58.default.bool,
  /**
   * If `true`, the selection on click on a row or cell is disabled.
   * @default false
   */
  disableRowSelectionOnClick: import_prop_types58.default.bool,
  /**
   * If `true`, the virtualization is disabled.
   * @default false
   */
  disableVirtualization: import_prop_types58.default.bool,
  /**
   * Controls whether to use the cell or row editing.
   * @default "cell"
   */
  editMode: import_prop_types58.default.oneOf(["cell", "row"]),
  /**
   * Unstable features, breaking changes might be introduced.
   * For each feature, if the flag is not explicitly set to `true`, the feature will be fully disabled and any property / method call will not have any effect.
   */
  experimentalFeatures: import_prop_types58.default.shape({
    ariaV7: import_prop_types58.default.bool,
    columnGrouping: import_prop_types58.default.bool,
    warnIfFocusStateIsNotSynced: import_prop_types58.default.bool
  }),
  /**
   * The milliseconds delay to wait after a keystroke before triggering filtering.
   * @default 150
   */
  filterDebounceMs: import_prop_types58.default.number,
  /**
   * Filtering can be processed on the server or client-side.
   * Set it to 'server' if you would like to handle filtering on the server-side.
   * @default "client"
   */
  filterMode: import_prop_types58.default.oneOf(["client", "server"]),
  /**
   * Set the filter model of the Data Grid.
   */
  filterModel: import_prop_types58.default.shape({
    items: import_prop_types58.default.arrayOf(import_prop_types58.default.shape({
    ...
    logicOperator: import_prop_types58.default.oneOf(["and", "or"]),
    quickFilterExcludeHiddenColumns: import_prop_types58.default.bool,
    quickFilterLogicOperator: import_prop_types58.default.oneOf(["and", "or"]),
    quickFilterValues: import_prop_types58.default.array
  }),
  /**
   * Forwarded props for the Data Grid root element.
   * @ignore - do not document.
   */
  forwardedProps: import_prop_types58.default.object,
  /**
   * Function that applies CSS classes dynamically on cells.
   * @param {GridCellParams} params With all properties from [[GridCellParams]].
   * @returns {string} The CSS class to apply to the cell.
   */
  getCellClassName: import_prop_types58.default.func,
  /**
   * Function that returns the element to render in row detail.
   * @param {GridRowParams} params With all properties from [[GridRowParams]].
   * @returns {React.JSX.Element} The row detail element.
   */
  getDetailPanelContent: import_prop_types58.default.func,
  /**
   ...
   */
  getEstimatedRowHeight: import_prop_types58.default.func,
  /**
   * Function that applies CSS classes dynamically on rows.
   * @param {GridRowClassNameParams} params With all properties from [[GridRowClassNameParams]].
   * @returns {string} The CSS class to apply to the row.
   */
  getRowClassName: import_prop_types58.default.func,
  /**
   * Function that sets the row height per row.
   * @param {GridRowHeightParams} params With all properties from [[GridRowHeightParams]].
   * @returns {GridRowHeightReturnValue} The row height value. If `null` or `undefined` then the default row height is applied. If "auto" then the row height is calculated based on the content.
   */
  getRowHeight: import_prop_types58.default.func,
  /**
   * Return the id of a given [[GridRowModel]].
   */
  getRowId: import_prop_types58.default.func,
  /**
   * Function that allows to specify the spacing between rows.
   * @param {GridRowSpacingParams} params With all properties from [[GridRowSpacingParams]].
   * @returns {GridRowSpacing} The row spacing values.
   */
  getRowSpacing: import_prop_types58.default.func,
  /**
   * If `true`, the footer component is hidden.
   * @default false
   */
  hideFooter: import_prop_types58.default.bool,
  /**
   * If `true`, the pagination component in the footer is hidden.
   * @default false
   */
  hideFooterPagination: import_prop_types58.default.bool,
  /**
   * If `true`, the selected row count in the footer is hidden.
   * @default false
   */
  hideFooterSelectedRowCount: import_prop_types58.default.bool,
  /**
   * If `true`, the diacritics (accents) are ignored when filtering or quick filtering.
   * E.g. when filter value is `cafe`, the rows with `café` will be visible.
   * @default false
   */
  ignoreDiacritics: import_prop_types58.default.bool,
  /**
   * The initial state of the DataGrid.
   * The data in it will be set in the state on initialization but will not be controlled.
   * If one of the data in `initialState` is also being controlled, then the control state wins.
   */
  initialState: import_prop_types58.default.object,
  /**
   * Callback fired when a cell is rendered, returns true if the cell is editable.
   * @param {GridCellParams} params With all properties from [[GridCellParams]].
   * @returns {boolean} A boolean indicating if the cell is editable.
   */
  isCellEditable: import_prop_types58.default.func,
  /**
   * Determines if a row can be selected.
   * @param {GridRowParams} params With all properties from [[GridRowParams]].
   * @returns {boolean} A boolean indicating if the cell is selectable.
   */
  isRowSelectable: import_prop_types58.default.func,
  /**
   ...
   * @default false
   */
  keepNonExistentRowsSelected: import_prop_types58.default.bool,
  /**
   ...
   * @default console
   */
  logger: import_prop_types58.default.shape({
    debug: import_prop_types58.default.func.isRequired,
    error: import_prop_types58.default.func.isRequired,
    info: import_prop_types58.default.func.isRequired,
    warn: import_prop_types58.default.func.isRequired
  }),
  /**
   * Allows to pass the logging level or false to turn off logging.
   * @default "error" ("warn" in dev mode)
   */
  logLevel: import_prop_types58.default.oneOf(["debug", "error", "info", "warn", false]),
  /**
   * Nonce of the inline styles for [Content Security Policy](https://www.w3.org/TR/2016/REC-CSP2-20161215/#script-src-the-nonce-attribute).
   */
  nonce: import_prop_types58.default.string,
  /**
   ...
   */
  onCellClick: import_prop_types58.default.func,
  /**
   ...
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onCellDoubleClick: import_prop_types58.default.func,
  /**
   * Callback fired when the cell turns to edit mode.
   * @param {GridCellParams} params With all properties from [[GridCellParams]].
   * @param {MuiEvent<React.KeyboardEvent | React.MouseEvent>} event The event that caused this prop to be called.
   */
  onCellEditStart: import_prop_types58.default.func,
  /**
   * Callback fired when the cell turns to view mode.
   * @param {GridCellParams} params With all properties from [[GridCellParams]].
   * @param {MuiEvent<MuiBaseEvent>} event The event that caused this prop to be called.
   */
  onCellEditStop: import_prop_types58.default.func,
  /**
   ...
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onCellKeyDown: import_prop_types58.default.func,
  /**
   * Callback fired when the `cellModesModel` prop changes.
   * @param {GridCellModesModel} cellModesModel Object containing which cells are in "edit" mode.
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onCellModesModelChange: import_prop_types58.default.func,
  /**
   * Callback called when the data is copied to the clipboard.
   * @param {string} data The data copied to the clipboard.
   */
  onClipboardCopy: import_prop_types58.default.func,
  /**
   ...
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onColumnHeaderClick: import_prop_types58.default.func,
  /**
   ...
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onColumnHeaderDoubleClick: import_prop_types58.default.func,
  /**
   ...
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onColumnHeaderEnter: import_prop_types58.default.func,
  /**
   ...
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onColumnHeaderLeave: import_prop_types58.default.func,
  /**
   ...
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onColumnHeaderOut: import_prop_types58.default.func,
  /**
   ...
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onColumnHeaderOver: import_prop_types58.default.func,
  /**
   ...
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onColumnOrderChange: import_prop_types58.default.func,
  /**
   * Callback fired when the column visibility model changes.
   * @param {GridColumnVisibilityModel} model The new model.
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onColumnVisibilityModelChange: import_prop_types58.default.func,
  /**
   * Callback fired when the Filter model changes before the filters are applied.
   * @param {GridFilterModel} model With all properties from [[GridFilterModel]].
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onFilterModelChange: import_prop_types58.default.func,
  /**
   ...
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onMenuClose: import_prop_types58.default.func,
  /**
   ...
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onMenuOpen: import_prop_types58.default.func,
  /**
   * Callback fired when the pagination model has changed.
   * @param {GridPaginationModel} model Updated pagination model.
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onPaginationModelChange: import_prop_types58.default.func,
  /**
   ...
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onPreferencePanelClose: import_prop_types58.default.func,
  /**
   ...
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onPreferencePanelOpen: import_prop_types58.default.func,
  /**
   * Callback called when `processRowUpdate` throws an error or rejects.
   * @param {any} error The error thrown.
   */
  onProcessRowUpdateError: import_prop_types58.default.func,
  /**
   ...
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onResize: import_prop_types58.default.func,
  /**
   ...
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onRowClick: import_prop_types58.default.func,
  /**
   * Callback fired when the row count has changed.
   * @param {number} count Updated row count.
   */
  onRowCountChange: import_prop_types58.default.func,
  /**
   ...
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onRowDoubleClick: import_prop_types58.default.func,
  /**
   * Callback fired when the row turns to edit mode.
   * @param {GridRowParams} params With all properties from [[GridRowParams]].
   * @param {MuiEvent<React.KeyboardEvent | React.MouseEvent>} event The event that caused this prop to be called.
   */
  onRowEditStart: import_prop_types58.default.func,
  /**
   * Callback fired when the row turns to view mode.
   * @param {GridRowParams} params With all properties from [[GridRowParams]].
   * @param {MuiEvent<MuiBaseEvent>} event The event that caused this prop to be called.
   */
  onRowEditStop: import_prop_types58.default.func,
  /**
   * Callback fired when the `rowModesModel` prop changes.
   * @param {GridRowModesModel} rowModesModel Object containing which rows are in "edit" mode.
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onRowModesModelChange: import_prop_types58.default.func,
  /**
   * Callback fired when the selection state of one or multiple rows changes.
   * @param {GridRowSelectionModel} rowSelectionModel With all the row ids [[GridSelectionModel]].
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onRowSelectionModelChange: import_prop_types58.default.func,
  /**
   * Callback fired when the sort model changes before a column is sorted.
   * @param {GridSortModel} model With all properties from [[GridSortModel]].
   * @param {GridCallbackDetails} details Additional details for this callback.
   */
  onSortModelChange: import_prop_types58.default.func,
  /**
   ...
   * @ignore - do not document.
   */
  onStateChange: import_prop_types58.default.func,
  /**
   * Select the pageSize dynamically using the component UI.
   * @default [25, 50, 100]
   */
  pageSizeOptions: import_prop_types58.default.arrayOf(import_prop_types58.default.oneOfType([import_prop_types58.default.number, import_prop_types58.default.shape({
   ...
   * @default "client"
   */
  paginationMode: import_prop_types58.default.oneOf(["client", "server"]),
  /**
   * The pagination model of type [[GridPaginationModel]] which refers to current `page` and `pageSize`.
   */
  paginationModel: import_prop_types58.default.shape({
    page: import_prop_types58.default.number.isRequired,
    pageSize: import_prop_types58.default.number.isRequired
  }),
  /**
   ...
   */
  processRowUpdate: import_prop_types58.default.func,
  /**
   * Number of extra rows to be rendered before/after the visible slice.
   * @default 3
   */
  rowBuffer: import_prop_types58.default.number,
  /**
   * Set the total number of rows, if it is different from the length of the value `rows` prop.
   * If some rows have children (for instance in the tree data), this number represents the amount of top level rows.
   */
  rowCount: import_prop_types58.default.number,
  /**
   * Sets the height in pixel of a row in the Data Grid.
   * @default 52
   */
  rowHeight: import_prop_types58.default.number,
  /**
   * Controls the modes of the rows.
   */
  rowModesModel: import_prop_types58.default.object,
  /**
   ...
   * @default 166
   */
  rowPositionsDebounceMs: import_prop_types58.default.number,
  /**
   * Set of rows of type [[GridRowsProp]].
   */
  rows: import_prop_types58.default.arrayOf(import_prop_types58.default.object).isRequired,
  /**
   * If `false`, the row selection mode is disabled.
   * @default true
   */
  rowSelection: import_prop_types58.default.bool,
  /**
   * Sets the row selection model of the Data Grid.
   */
  rowSelectionModel: import_prop_types58.default.oneOfType([import_prop_types58.default.arrayOf(import_prop_types58.default.oneOfType([import_prop_types58.default.number, import_prop_types58.default.string]).isRequired), import_prop_types58.default.number, import_prop_types58.default.string]),
  /**
   * Sets the type of space between rows added by `getRowSpacing`.
   * @default "margin"
   */
  rowSpacingType: import_prop_types58.default.oneOf(["border", "margin"]),
  /**
   * Number of rows from the `rowBuffer` that can be visible before a new slice is rendered.
   * @default 3
   */
  rowThreshold: import_prop_types58.default.number,
  /**
   * Override the height/width of the Data Grid inner scrollbar.
   */
  scrollbarSize: import_prop_types58.default.number,
  /**
   * If `true`, the vertical borders of the cells are displayed.
   * @default false
   */
  showCellVerticalBorder: import_prop_types58.default.bool,
  /**
   * If `true`, the right border of the column headers are displayed.
   * @default false
   */
  showColumnVerticalBorder: import_prop_types58.default.bool,
  /**
   * Overridable components props dynamically passed to the component at rendering.
   */
   ...
   * @default "client"
   */
  sortingMode: import_prop_types58.default.oneOf(["client", "server"]),
  /**
   * The order of the sorting sequence.
   * @default ['asc', 'desc', null]
   */
  sortingOrder: import_prop_types58.default.arrayOf(import_prop_types58.default.oneOf(["asc", "desc"])),
  /**
   * Set the sort model of the Data Grid.
   */
  sortModel: import_prop_types58.default.arrayOf(import_prop_types58.default.shape({
    field: import_prop_types58.default.string.isRequired,
    sort: import_prop_types58.default.oneOf(["asc", "desc"])
  })),
  /**
   * The system prop that allows defining system overrides as well as additional CSS styles.
   */
   ...
   * @default false
   */
  unstable_ignoreValueFormatterDuringExport: import_prop_types58.default.oneOfType([import_prop_types58.default.shape({
    clipboardExport: import_prop_types58.default.bool,
    csvExport: import_prop_types58.default.bool
